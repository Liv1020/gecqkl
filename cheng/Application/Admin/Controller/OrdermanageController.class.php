...
class OrdermanageController extends Controller{
	...
	*/
	public function index(){
		...
		$datatotal = $orderslist->field('count(*) count')->where(array('username' => $_SESSION['uname'],"o_type"=>0))->find();
		// 计算满足条件的数据总数
	...
	*/
	public function nonDelivery(){
		...
		$orderslist = M("orders");
		// 遍历出来的是一维数组。select遍历的二维数组。
		$datatotal = $orderslist->field('count(*) count')->where(array('username' => $_SESSION['uname'],"o_type"=>0,'status' => 1))->find();
		// 计算满足条件的数据总数
		$totalnums = $datatotal['count'];
		// 实例化分页类
		$pages = new \Think\Page($totalnums,10);
		...
		// 全部订单
		// 查询订单用户信息
		$orders_data = $orderslist ->where(array('username' => $_SESSION['uname'],"o_type"=>0,'status' => 1))->limit($limit)->select();
	...
	public function deliverGoods(){
		...
		$orderslist = M("orders");
		// 遍历出来的是一维数组。select遍历的二维数组。
		$datatotal = $orderslist->field('count(*) count')->where(array('username' => $_SESSION['uname'],"o_type"=>0,'status' => 2))->find();
		// 计算满足条件的数据总数
		$totalnums = $datatotal['count'];
		// 实例化分页类
		$pages = new \Think\Page($totalnums,10);
		...
		// 全部订单
		// 查询订单用户信息
		$orders_data = $orderslist ->where(array('username' => $_SESSION['uname'],"o_type"=>0,'status' => 2))->limit($limit)->select();
	...
	public function completeDeal(){
		...
		$orderslist = M("orders");
		// 遍历出来的是一维数组。select遍历的二维数组。
		$datatotal = $orderslist->field('count(*) count')->where(array('username' => $_SESSION['uname'],"o_type"=>0,'status' => 3))->find();
		// 计算满足条件的数据总数
		$totalnums = $datatotal['count'];
		// 实例化分页类
		$pages = new \Think\Page($totalnums,10);
		...
		// 全部订单
		// 查询订单用户信息
		$orders_data = $orderslist ->where(array('username' => $_SESSION['uname'],"o_type"=>0,'status' => 3))->limit($limit)->select();
	...
	public function search(){
		...
		// 全部订单
		// 查询订单信息,"o_type"=>0
		$orders_data = $list->table('sx_member m,sx_orders o')->field('o.oid,o.onumber,o.otime,o.total,m.uname,o.deliveryaddress')->where('o.o_type=0 and o.uid=m.uid and and o.username='.$_SESSION['uname'].' (o.onumber like %'.$_GET['uname'].'%)')->limit($limit)->select();
		// 得打发往前台的中间数据，订单详情表
