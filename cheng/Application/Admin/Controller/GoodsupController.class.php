...
class GoodsUpController extends Controller{
	...
	*/
	public function modGoods(){
		...
		$mod = M('goods');
		// 查询出要修改数据的字段
		$goodsdata = $mod->field('gid,gclassification,gname,gattribute,goods_spec,goldprice,goodnums,gpic,gintroduce,gspecifications,issale')->where(array('gid'=>$gid,'username'=>$_SESSION['uname']))->find();
		if(empty($goodsdata)){
			$this -> error('非法操作');
		}else{
