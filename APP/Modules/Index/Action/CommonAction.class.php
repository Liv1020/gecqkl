<?php  
	...
	public function sign(){
		...
		$mid=$_SESSION['mid'];
		$mem=$member->where(array("id"=>$mid))->find();
		$a=date("Y-m-d",time());
		$b=date("Y-m-d",$mem['sign_time']);
		$day=$this->diffBetweenTwoDays($a,$b);
		if($day >1){
			$member->where(array("id"=>$mid))->save(array('sign_time'=>time()));
			$member->where(array("id"=>$mid))->setInc('shengchanli',5);
			//$order->where(array("user_id"=>$mid))->setInc('lixi',1);
			$powder_log->add(array("user_id"=>$mid,'reason'=>'签到','state'=>1,'val'=>5,'add_time'=>date('Y-m-d H:i')));
			//$this->success("签到成功！",U("index/index"));
		}
		/*else{

			$this->error("今天已签到，请明天在来！",U("index/index"));
		}*/
