		  ...
		  if (IS_POST) {
		   ...
		   $data['idcard']     = I('post.idcard','','htmlspecialchars');
            ...
            if (M('member')->where(array('username'=>session('username')))->save($data)) {
            	...
            	M('sclhquqq')->add(array('user_id'=>$_SESSION['mid'],'scl'=>100,'rem'=>'注册赠送','add_time'=>$addtime,'end_time'=>$endtime));
            	$product=M('product')->where(array("id"=>1))->find();
            	
            	$ord["user"] = $_SESSION['username'];
            	$ord["user_id"] = $_SESSION['mid'];
				...
				$this->success('资料提交成功！',U('Index/index/index'));
				exit;
