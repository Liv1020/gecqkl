<?php  
	...
	public function zzchurang(){
		...
		if(IS_POST){
			...
			$data['zz_num']=$_POST['num'];
			$data['sum_price']=$_POST['price']*$_POST['num'];
			$data['add_time']=time();
			$res=$zztrans->add($data);
			if($res){
				$member->where(array('id'=>$data['user_id']))->setDec('jinbi',$data['zz_num']);
				$this->success("交易添加成功！",U("index/seed/crlist"));
			}
			else{
				$this->error("交易添加失败！");
			}
		...
		$this->assign('gaojia',$gaojia);
		$this->assign('dijia',$dijia);
	 ...
	 * 种子交易列表
	 */
	public function zzjylist(){
		$member=M("member");
		$zztrans=M("zztrans");
		$zzlist=$zztrans->where(array("trans_state"=>1))->select();
		
		$this->assign("zzlist",$zzlist);
		$this->display();
	}
	/**
	 * 种子交易完成列表
	 */
	public function zzwclist(){
		$member=M("member");
		$zztrans=M("zztrans");
		$zzlist=$zztrans->where(array("trans_state"=>4))->select();
	 ...
	 * ajax种子交易
	 */
	public function ajax_zzjy(){
		$member=M("member");
		$zztrans=M("zztrans");
		$id=$_POST['id'];
		$user_id=$_SESSION['mid'];
		$zz=$zztrans->where(array("id"=>$id))->find();
		//$user=$member->where(array("id"=>$zz['user_id']))->find();
		if($user_id == $zz['user_id']){
			$res['result'] = 2;
			$res['msg'] = '不可购买自己的种子！';
		}
		else{
			$res['result'] = 1;
			$res['id'] = $zz['id'];
			$res['sum_price'] = $zz['sum_price'];
			$res['zz_num'] = $zz['zz_num'];
			$res['zz_price'] = $zz['zz_price'];
			$res['zz_num'] = $zz['zz_num'];
		}
	...
	public function ntlist(){
		...
		$user_id = $_SESSION['mid'];
		$nt = $order->where(array("user_id"=>$user_id,"end_time"=>array("gt",time())))->select();
		foreach($nt as $key=>$val){
			$nt[$key]['yxtime'] = $this->timediff($val['UG_getTime'],time());
			$nt[$key]['sytime'] = $this->timediff(time(),$val['end_time']);
		}
	...
	public function daili(){
		...
		if(IS_POST){
			...
			$data['area_id'] = $_POST['area3'];
			$data['name'] = $_POST['name'];
			$data['tel'] = $_POST['tel'];
			$data['add_time'] = time();
			$res = $daili->add($data);
			if($res){
				$this->success("申请代理成功！",U("index/emoney/daili"));
			}
			else{
				$this->error("申请代理失败！");
			}
		...
		$dlinfo = $daili->where(array("user_id"=>$user_id))->find();
		if($dlinfo){
			$area3 = $area->where(array("area_id"=>$dlinfo['area_id']))->find();
			...
			$dllst = $daili->where()->select();
			foreach($dllst as $key=>$val){
				$area_ids[] = $val['area_id'];
			}
			$arealst = $area->where(array("area_parent_id"=>0,"id"=>array('not in',$area_ids)))->select();
			$this->assign("arealst",$arealst);
		}
		$this->assign('adv_list',$adv_list);
		$this->display();
	}
	...
	public function ajaxarea(){
		...
		$arealst = $area->where(array("area_parent_id"=>$id,"id"=>array('not in',$area_ids)))->select();
		echo json_encode($arealst);
	}
	/**
	 * 我的粉丝
	 * 
	...
	public function fensi(){
		...
		$announce = M("announce");
		$user_id = $_SESSION['mid'];
		$mem_list = $member->where(array("parent_id"=>$user_id))->select();
		$anninfo = $announce->where(array("tid"=>14))->find();
...
public function myjiaoyis(){
	...
	public function shouye(){
		...
		$jylist = $zztrans->where(array("trans_state"=>1))->limit(0,10)->select();
		...
		$u_list = $member->where(array("id"=>array("not in","3,8,9,11,13,14")))->order("jinbi desc")->limit(0,10)->select();
...
     * @author 月月
	 ...
	 * ajax收种子动作
	 */
	public function ajaxshouzz(){
		$user_id=$_SESSION['mid'];
		$member=M("member");//setDec
		$user_id=$_SESSION['mid'];
		$res=$member->where(array("id"=>$user_id))->setDec('sqczcs');
		if($res){
			echo 1;
		}
		else{
	 ...
	 */
	public function ajaxsshouqu(){
		$id=$_POST['id'];
		$yieldid=$_POST['yieldid'];
		$seed_log=M('seed_log');
		$user_id=$_SESSION['mid'];
		$member=M("member");
		$yield=M('yield');
