	...
	*/
	class MemberAction extends CommonAction{
		...
		public function grade(){
			...
			$count = $grade -> where()->count();
			$Page       = new Page($count,10);
			$show = $Page -> show();
			$list = $grade -> where() -> limit($Page ->firstRow.','.$Page -> listRows)-> select();
		...
		public function addgrade(){
			$grade = M("grade");
			if(IS_POST){
				$grmin = $grade->where()->field("max(vip_val) as vip_val")->find();
				$data['vip_name'] = $_POST['vip_name'];
				$data['vip_val'] = $grmin['vip_val']+1;
				$data['add_time'] = date("Y-m-d H:i:s");
				$res = $grade->add($data);
				if($res){
					$this->success("添加成功",U('member/grade'));
				}
				else{
					$this->error("添加失败！");
				}
		...
		public function daili(){
	    	...
	    	$member = M("member");
	    	import("@.ORG.Util.Page");
			$count = $daili -> where()->count();
			$Page       = new Page($count,10);
			$show = $Page -> show();
			$list = $daili -> where()->order("add_time desc") -> limit($Page ->firstRow.','.$Page -> listRows)-> select();
			foreach($list as $key=>$val){
				$user = $member->where(array("id"=>$val['user_id']))->find();
				$area3 = $area->where(array("area_id"=>$val['area_id']))->find();
				$area2 = $area->where(array("area_id"=>$area3['area_id']))->find();
				$area1 = $area->where(array("area_id"=>$area2['area_id']))->find();
				$list[$key]['area1'] = $area1['area_name'];
				$list[$key]['area2'] = $area2['area_name'];
				$list[$key]['area3'] = $area3['area_name'];
				$list[$key]['user_name'] = $area3['username'];
		 ...
		 * 代理通过
		 */
		public function dlstat(){
			$id = $_GET['id'];
			if($id){
	    		$daili = M("daili");
	    		$res = $daili->where(array("id"=>$id))->save(array("state"=>1));
	    		if($res){
	    			$this->success("已通过！",U('member/daili'));
	    		}
	    		else{
	    			$this->error("操作失败！");
	    		}
		 ...
		 * 取消该代理
		 */
		public function dldel(){
			$id = $_GET['id'];
			if($id){
	    		$daili = M("daili");
	    		$res = $daili->where(array("id"=>$id))->delete();
	    		if($res){
	    			$this->success("已通过！",U('member/daili'));
	    		}
	    		else{
		...
		public function delgrade(){
			$grade = M("grade");
			$id = $_GET['id'];
			$grade->where(array("id"=>array('gt',$id)))->setDec("vip_val");
			$res = $grade->where(array("id"=>$id))->delete();
			if($res){
				$this->success("删除成功！",U('member/grade'));
			}
			else{
				$this->error('删除失败！');
			}
		...
		public function vipgods(){
			$vipgods = M("vipgods");
			import("@.ORG.Util.Page");
			$count = $vipgods -> where()->count();
			$Page       = new Page($count,10);
			$show = $Page -> show();
			$list = $vipgods -> where() -> limit($Page ->firstRow.','.$Page -> listRows)-> select();
			$this->assign("list",$list);
			$this->assign("page",$show);
			$this->display();
		}
		 ...
		 * VIP商品添加
		 */
		public function addvipgods(){
			$grade = M("grade");
			$vipgods = M("vipgods");
			$vipdj = $grade->where()->select();
			if(IS_POST){
				$count=0;
	            ...
	            foreach ($_FILES["g_pic"]["error"] as $key => $error){
	                if($error == 0){
	                    $tmp_namey = $_FILES["g_pic"]["tmp_name"][$key];
	                    $ay=explode(".",$_FILES["g_pic"]["name"][$key]);  //截取文件名跟后缀
	                    $prename = $ay[0];
	            ...
	            $data['g_price'] = $_POST['g_price'];
	            $data['g_vip'] = $_POST['g_vip'];
	            $data['V1'] = $_POST['V1']/100;
	            $data['V2'] = $_POST['V2']/100;
	            $data['V3'] = $_POST['V3']/100;
	            $data['V4'] = $_POST['V4']/100;
	            $data['V5'] = $_POST['V5']/100;
	            $data['V6'] = $_POST['V6']/100;
	            $data['g_content'] = $_POST['g_content'];
	            $data['g_pic'] = $goodsimgs;
	            $data['add_time'] = date("Y-m-d H:i:s");
	            $res = $vipgods->add($data);
				if($res){
					$this->success("添加成功！",U("member/vipgods"));
				}
				else{
					$this->error("添加失败！");
				}
			...
			$this->assign("vipdj",$vipdj);
			$this->display();
		}
		/**
		 * 删除VIP商品
		 */
		public function delvipgods(){
			$id = $_GET['id'];
			$vipgods = M("vipgods");
			$res = $vipgods->where(array("id"=>$id))->delete();
			if($res){
				$this->success("删除成功！",U("Member/vipgods"));
			}
			else{
				$this->error("删除失败！");
			}
		...
		public function viporder_list(){
	    	...
	    	$count = $orders->where($where)->count(); // 查詢滿足要求的總記錄數
	    	$p = new Page($count,20);
			$list = $orders->where($where)->order("oid desc")->limit($p->firstRow, $p->listRows)->select();
			$show  = $p->show();// 分页显示输出
			$this->assign("list",$list);
			$this->assign("page",$show);
			$this->display();
		 ...
		 * VIP订单发货
		 */
		public function viporder_deliver(){
			$orders = M("orders");
			$id = $_GET['id'];
			if($id){
				$info = $orders->where(array("oid"=>$id))->find();
				if(IS_POST){
					$id = $_POST['id'];
					$data['kuaidiname'] = $_POST['kuaidiname'];
					$data['expressnum'] = $_POST['expressnum'];
					$data['status'] = 2;
					$res = $orders->where(array("oid"=>$id))->save($data);
					if($res){
						$this->success("发货成功！",U('member/viporder_list'));
					}
					else{
						$this->error("发货失败！");
					}
