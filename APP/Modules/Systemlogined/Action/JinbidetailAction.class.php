...
class JinbidetailAction extends CommonAction {
	...
	public function weishouzongzi(){
		$member=M('member');
		$yield=M("yield");
		$order=M("order");
		import("@.ORG.Util.Page");// 导入分页类    	   	
    	$count = $member->where()->count(); // 查詢滿足要求的總記錄數
    	$p = new Page($count,50);
		
		$mem_list=$member->field("id,username")->limit ( $p->firstRow, $p->listRows )->select();
		
		
    	
		foreach($mem_list as $key=>$val){
			$contzz=$yield->field("count(*) as num,time")->where(array("user_id"=>$val['id']))->find();
			$ord=$order->field("UG_getTime")->where(array("user_id"=>$val['id']))->find();
			$mem_list[$key]['zznum']=$contzz['num'];
			$mem_list[$key]['time']=$ord['UG_getTime'];
		}
		 $show       = $p->show();// 分页显示输出
	     $this->assign('page',$show);// 赋值分页输出		
	...
	public function weishouinfo(){
		$yield=M("yield");
		$user_id=$_REQUEST['user_id'];
		$yie_ld=$yield->where(array("user_id"=>$user_id))->select();
		$this->assign("yie_ld",$yie_ld);
